...
class SiamBANTracker(SiameseTracker):
    ...
    def init(self, img, bbox):
        ...
        c_x = (cfg.TRACK.INSTANCE_SIZE - 1) / 2
        roi_x = torch.tensor([[c_x - bbox[2] * scale / 2, c_x - bbox[3] * scale / 2,
                             c_x + bbox[2] * scale / 2, c_x + bbox[3] * scale / 2]])
        self.roi_x=roi_x
        ...
        self.model.template(z_crop,x_crop,roi_x)
        self.cls_score=1
        '''
    ...
    def track(self, img):
        ...
        #print("cls score:",score.max())

        '''
        cls_show=outputs['cls'].detach()
        b, a2, h, w = cls_show.size()
      #  cls_show = cls_show.view(b, 2, a2//2, h, w)
        cls_show = cls_show.permute(0, 2, 3, 1).contiguous()
        cls_show = F.softmax(cls_show, dim=3)
       
        cls_show = cls_show[0,:,:,1]
        cls_show=cls_show.cpu().numpy().squeeze()
        
       # print(cls_show.shape)
     #   cls_show = F.softmax(cls_show[:,:,:,:], dim=1).data[:,1,:,:].cpu().numpy().squeeze()
        
        if self.idx>0 and self.idx<80:
            viz = Visdom(env='cls1 map')
            first_cls_value_show=cls_show
           # viz.surf(X=first_cls_value_show, opts=dict(colormap='Hot'))
            viz.heatmap(first_cls_value_show)
        '''
