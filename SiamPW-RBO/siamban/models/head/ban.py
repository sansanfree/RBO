...
class DepthwiseXCorr(nn.Module):
    def __init__(self, in_channels, hidden, out_channels, kernel_size=3):
       ...
       # self.xorr_kernel= nn.Sequential(
       #         nn.Conv2d(in_channels, hidden, kernel_size=5, bias=False),
       #         nn.BatchNorm2d(hidden),
       #         nn.ReLU(inplace=True),
       #         )
      #  self.xorr_search = nn.Conv2d(hidden, hidden, kernel_size=5, bias=False)

        self.xorr_search = nn.Sequential(
        ...
        self.head = nn.Sequential(
                nn.Conv2d(hidden, hidden, kernel_size=3, bias=False),
                nn.BatchNorm2d(hidden),
                nn.ReLU(inplace=True),
    ...
    def forward(self, kernel, search):
        ...
        kernel=self.xorr_kernel(kernel)
       # print("search shape:",search.shape)
        #print("kernel shape:",kernel.shape)
       # feature = xcorr_depthwise(search, kernel)
        feature =search*kernel
      #  print("feature before shape:",feature.shape)
        feature=self.xorr_search(feature)
      #  print("feature after shape:",feature.shape)
